...
int activityNotifications(vector<int> expenditure, int d) {
    auto median2 = [d](array<int, 201> v) {
      if (d % 2) {
        auto mid = d / 2 + 1;
        for (int i = 0; i < 201; i++) {
          if (v[i] > 0) {
            if (v[i] >= mid) return i * 2;
            mid -= v[i];
          }
        }
      } else {
        ...
        for (int i = 0; i < 201; i++) {
          if (v[i] > 0) {
            if (v[i] >= mid1 && mid1 > 0) median += i;
            if (v[i] >= mid2) return median + i;
            mid1 -= v[i];
            mid2 -= v[i];
          }
        }
  ...
  auto ans = 0;
  array<int, 201> v{};
  for (auto i = 0; i < d; i++) v[expenditure[i]]++;
  for (int i = d; i < expenditure.size(); i++) {
    if (expenditure[i] >= median2(v)) ans++;
    v[expenditure[i - d]]--;
    v[expenditure[i]]++;
  }
  return ans;
...
void test_main(const string &input_file, const string &result_file) {
  ...
  int result = activityNotifications(ar, r);
